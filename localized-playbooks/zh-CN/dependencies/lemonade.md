...
如果你看到了版本号，说明 Lemonade 已正确安装并可以使用。

为了快速参考，下面是常用的 Lemonade CLI 命令：

| 命令 | 作用 |
|------|------|
| `lemonade --help` | 显示所有可用命令和参数。 |
| `lemonade --version` | 打印已安装的 Lemonade 版本。 |
| `lemonade status` | 确认 Lemonade server 是否正在运行且可访问。默认 OpenAI 兼容 API 基础 URL 为 `http://localhost:13305/api/v1`。 |
| `lemonade list` | 列出 Lemonade 设置中可用的模型。 |
| `lemonade pull <MODEL_NAME>` | 下载模型但不启动。 |
| `lemonade run <MODEL_NAME>` | 如有需要先下载模型，然后启动模型进行推理或聊天。 |
...
如需了解最新 Lemonade server 选项或故障排查，请参考 [Lemonade 官方文档](https://lemonade-server.ai/docs/lemonade-cli/)。
